...
class RIPointTransformer(nn.Module):
    def __init__(self, blocks=[2, 3, 3, 3], block=RIPointTransformerBlock, c=1, transformer_architecture=None, with_cross_pos_embed=None, factor=1, occ_thres=0.):
        ...
        ## for Segmentation
        self.cls = nn.Sequential(nn.Linear(planes[0], planes[0]), nn.BatchNorm1d(planes[0]), nn.ReLU(inplace=True), nn.Linear(planes[0], 15))
    ...
    def forward(self, pxon):
        ...
        p0, x0, o0, n0 = pxon  # (n, 3), (n, c), (b), (n, 4)
        ############################
        # encoder
        ############################
        # source pcd
        ...
        ###########################
        nr, pr = group_all(n4), group_all(p4)
        ppf = calc_ppf_gpu(p4, n4, pr, nr)  # (n, nsample, 4)
        ...
        x4 = self.dec4[1:]([p4, self.dec4[0]([p4, x4, o4]), o4, n4, idx4, ppf4, None])[1]
        ...
        # d_idx3 = d_idx2[d_idx3.long()]
        # d_idx4 = d_idx3[d_idx4.long()].long()
        # p4 = src_deformed_pcd[d_idx4]
        
        # return s_p4, s_g_x4, src_deformed_pcd, s_x1, t_p4, t_g_x4, t_p1, t_x1
        # return p4, src_deformed_pcd, x1